...
// 顶栏权限模式切换：每个对话独立，下拉选择 acceptEdits/plan/bypassPermissions
export function ModeSwitcher({ convId }: { convId: string | null }) {
  const [modes, setModes] = useState<ModelItem[]>([]);
  const [current, setCurrent] = useState<string>('acceptEdits');
  ...
  useEffect(() => {
    ...
    window.claude.getMode().then(setCurrent).catch(() => {});
  }, [convId]);
  ...
  const pick = async (alias: string) => {
    const next = await window.claude.setMode(alias);
    setCurrent(next);
    setOpen(false);
  };

  const label = modes.find((m) => m.alias === current)?.name ?? current;
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        <Icon name="gear" size={13} color="var(--green)" />
        <span>{label}</span>
